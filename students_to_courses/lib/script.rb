...
def solve_with_minizinc(rendered_file_name, time_as_string, watch)
  ...
  watch.start
  system "mzn-g12fd  #{rendered_file_name} > #{result_file}"                   #--all-solutions
...
def solve_with_yices(rendered_file_name, time_as_string, watch)
  ...
  watch.start
  system "./yices.sh mzn/#{rendered_file_name} > #{result_file}"
  puts time_as_string +" - "+watch.tick
  Dir.chdir ".."
  watch.reset
end
...
1.times do
  ...
  Builder.new([40,20,20], [50, 0, 0]).create         #geht eeewig
...
#  Builder.new([20,20,60], [20, 20, 20]).create       # < 2 s
  Builder.new([15,10,4,10,20,10,10,10,10], [11, 11, 11, 11, 11]).create




  #Builder.new([20,20,20], [20, 20, 20]).create
  #Builder.new([10,10,10], [10, 10, 10]).create     #blitzschnell
...
#  Builder.new([10,4,5, 12, 20, 9, 7, 11, 33], [4]).create         #geht eeewig
  #Builder.new([1,1,1], [2]).create         #geht eeewig

  info = Info.create

  t = Time.now
  time_as_string = t.strftime("%y_%m_%d_%H_%M_%S_%L")
  template_file_name = "mzn_template.mzn.erb"
  rendered_file_name = "students-to-courses_#{time_as_string}.mzn"
...
#  solve_with_minizinc(rendered_file_name, time_as_string, watch)
